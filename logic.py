...
class SplashScreen:
    def __init__(self, screen,
        ...
        # Исправленные пути к файлам (относительно корня проекта)
        default_pages = [
        ...
        default_sound_path = os.path.join(script_dir,
                                          "Main/soundtrack/Undertale_Soundtrack_-_1_Once_Upon_a_Time_Undertale_OST_70348777.mp3")
        default_line_spacing = 20
        default_image_size = (500, 290)
        default_image_pos = None
        default_text_start_pos = None
        default_font_path = os.path.join(script_dir, "fonts/SMB1NESClassix-Regular.otf")
        default_typing_sound_path = os.path.join(script_dir, "soundtrack/SND_TXT2.wav")
        default_page_font_sizes = [17] * 7 + [14] + [17] * 6
        default_page_colors = [(255, 255, 255)] * 7 + [(255, 200, 100)] + [(255, 255, 255)] * 6
        default_page_text_align = ["center"] * 14  # Все страницы с центрированием
        ...
        self.page_text_align = self._fix_list_length(self.page_text_align, num_pages, "center")
        ...
        if self.image_pos is None:
            self.image_center = screen_rect.center
        else:
            self.image_center = self.image_pos

        if self.text_start_pos is None:
            # Текст размещаем под изображением
            if self.image_size:
                text_y = screen_rect.centery + self.image_size[1] // 2 + 50
            else:
                text_y = screen_rect.centery + 100
            self.text_start_pos = (screen_rect.centerx, text_y)
        else:
            self.text_start_pos = self.text_start_pos
        ...
        for i, path in enumerate(self.image_paths):
            if path and os.path.exists(path):
                try:
                    ...
                    print(f"Загружено изображение {i}: {path}")
        ...
        if self.sound_path and os.path.exists(self.sound_path):
            try:
                ...
                print(f"Загружен звук: {self.sound_path}")
            except:
                print(f"Не удалось загрузить звук: {self.sound_path}")
        ...
        if self.typing_sound_path and os.path.exists(self.typing_sound_path):
            try:
                ...
                print(f"Загружен звук печати: {self.typing_sound_path}")
            except:
                print(f"Не удалось загрузить звук печати: {self.typing_sound_path}")
    ...
    def show(self):
        ...
        skip_intro = False
        esc_pressed = False  # Флаг для отслеживания нажатия ESC
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE and not esc_pressed:
                        esc_pressed = True
                        skip_intro = True
                        # Останавливаем музыку при пропуске
                        if self.sound:
                            self.sound.stop()
                        # Выходим из цикла
                        running = False
        ...
        # Убеждаемся, что музыка остановлена
        if self.sound:
            self.sound.stop()
