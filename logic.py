...
class SplashScreen:
    def __init__(self, screen,
        ...
        default_image_paths = [
            "texture/SplashScreen/TwoRace.jpg",
            "texture/SplashScreen/TwoRace.jpg",
            "texture/SplashScreen/2.jpg",
            "texture/SplashScreen/3.jpg",
            "texture/SplashScreen/3.1.jpg",
            None,
            None,
            "texture/SplashScreen/FEFUT.jpg",
            "texture/SplashScreen/4.jpg",
            "texture/SplashScreen/5.jpg",
            "texture/SplashScreen/6.jpg",
            "texture/SplashScreen/7.jpg",
            "texture/SplashScreen/7.1.jpg",
            "texture/SplashScreen/8.jpg",
        ]
        default_font_size = 17
        default_color = (255, 255, 255)
        default_bg_color = (0, 0, 0)
        default_sound_path = "soundtrack/Undertale_Soundtrack_-_1_Once_Upon_a_Time_Undertale_OST_70348777.mp3"
        default_line_spacing = 20
        default_image_size = (500, 290)
        default_image_pos = (400, 250)
        default_text_start_pos = (100, 450)
        default_font_path = "fonts/SMB1NESClassix-Regular.otf"
        default_typing_sound_path = "soundtrack/SND_TXT2.wav"
...
'''
НИЖЕ КЛАСС ДЛЯ ИНИЦИАЛИЗАЦИИ БОЯ (ВОЗМОЖНО ТРЕБУЮТСЯ ИЗМЕНЕНИЯ, АКТИВАЦИЯ В ФАЙЛЕ test_battle.py)
'''
...
class BattleUI:
    ...
    def __init__(self, screen,
        ...
        # Сохраняем исходную Y-координату основных кнопок
        self.original_main_buttons_y = self.main_button_rects[0].y

        self.text_area_rect = pygame.Rect(
            self.button_spacing,
            start_y - self.text_area_height - self.text_area_bottom_margin,
        ...
        self.selected_type = 'main'   # 'main' или 'popup'
        self.selected_index = 0
        self.using_keyboard = False   # последнее управление с клавиатуры
        ...
        # Флаг блокировки ввода (кроме ESC)
    ...
    # ----- Методы для управления позицией основных кнопок -----
    def set_main_buttons_y(self, y):
        """Устанавливает новую Y-координату для всех основных кнопок."""
    ...
    def set_popup_buttons(self, button_texts, outline_colors=None, layout='horizontal', padding=10):
        ...
        # При появлении всплывающих кнопок выбираем первую из них
    ...
    def clear_popup_buttons(self):
        ...
        self.popup_button_outline_colors = []
        self.hovered_popup = -1
        # Возвращаем выбор на основные кнопки
        self.selected_type = 'main'
        self.selected_index = 0
        self.using_keyboard = True

    def handle_event(self, event):
        # Если ввод заблокирован, разрешаем только ESC
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            for i, rect in enumerate(self.main_button_rects):
                if rect.collidepoint(event.pos):
                    ...
                    self.using_keyboard = False
                    return -1
            # Если мышь не над кнопками – оставляем выделение как было
            return -1
    ...
    def draw(self):
        ...
        # Всплывающие кнопки
        for i, rect in enumerate(self.popup_button_rects):
            # Определяем цвет текста и обводки
...
'''
НИЖЕ КЛАСС ДЛЯ САМОГО БОЯ, КОТОРЫЙ КАК БЫ "НАСЛЕДУЕТСЯ" ОТ BattleUI (ТРЕБУЮТСЯ ИЗМЕНЕНИЯ, АКТИВАЦИЯ В ФАЙЛЕ test_battle.py)
'''

class BattleMiniGame:
    def __init__(self, battle_ui, arena_rect, duration=10000):
        self.ui = battle_ui
        self.arena = arena_rect.copy()
        self.duration = duration  # в миллисекундах
        self.start_time = pygame.time.get_ticks()
        
        # Параметры игрока (красный шарик)
        self.player_radius = 15
        self.player_pos = [self.arena.centerx, self.arena.centery]
        self.player_color = (255, 0, 0)  # красный
        self.player_speed = 5
        
        # Для ограничения движения игрока
        ...
        self.finished = False
        self.victory = False  # по истечении времени считаем, что игрок выжил?
    
    def handle_event(self, event):
        # Обрабатываем ESC для выхода
        if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
            self.finished = True
            self.victory = False
        # Другие события игнорируем (движение через update)
    ...
    def update(self):
        ...
        if elapsed >= self.duration:
            self.finished = True
            self.victory = True  # выжил 10 секунд - победа
            return
        ...
        # Применяем движение
        if dx != 0 or dy != 0:
            # Нормализация диагональной скорости (чтобы не было быстрее)
            if dx != 0 and dy != 0:
                dx *= 0.7071  # 1/√2
                dy *= 0.7071
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(screen, self.player_color,
                           ...
                           self.player_radius)
        # Можно показать оставшееся время
        elapsed = pygame.time.get_ticks() - self.start_time
