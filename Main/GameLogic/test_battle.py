...
def main():
    ...
    battle_ui.set_initial_message("Появился МАЛЫШЕВ!")
    boss.show_dialogue([
    ...
    def start_boss_attack():
        ...
        if boss.patterns[boss.phase]:
            pattern_name = random.choice(boss.patterns[boss.phase])
        else:
            pattern_name = "Базовая атака"

        arena = battle_ui.text_area_rect.copy()

        if pattern_name == "Движение стены":
            minigame = MovingWallGame(
                battle_ui, arena,
                duration=10000,
                enemy_damage=3,
                damage_interval=300
            )
            battle_ui.add_message("МАЛЫШЕВ: Двигайся A/D чтобы увернуться!")
        elif pattern_name == "Кости из пола":
            minigame = BonesFromFloorGame(
                battle_ui, arena,
                duration=10000,
                enemy_damage=2,
                damage_interval=200
            )
            battle_ui.add_message("МАЛЫШЕВ: Смотри под ноги! WASD спасут тебя!")
        else:
            minigame = DummyGame(
                battle_ui, arena,
                duration=10000,
        ...
        boss.start_attack(pattern_name, minigame)
    ...
    while running:
        ...
        if current_state == STATE_BATTLE and boss.current_minigame:
            if boss.current_minigame.is_finished():
                ...
                if boss.current_minigame.is_victory():
                    battle_ui.add_message("Отлично! Ты успешно уклонился!")
                    boss.show_dialogue([
                        "Хорошая реакция!",
